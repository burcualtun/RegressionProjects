...
def grab_outliers(dataframe, col_name, index=False):
    ...
    if dataframe[((dataframe[col_name] < low) | (dataframe[col_name] > up))].shape[0] > 10: # show top 5 if number of rows is greater than 10
        print(dataframe[((dataframe[col_name] < low) | (dataframe[col_name] > up))].head())
    else:
        print(dataframe[((dataframe[col_name] < low) | (dataframe[col_name] > up))]) # Show all if less than 10
